...
//count the connectivity parts 
   ...
   void findPathDijkstra(int start,int des, RandomAccessFile f) throws E> son

//initiation
int check[]=new int[n]; //kiem tra dinh da xet
int d[]=new int[n]; // khoang cach ngan nhat hien tai

int pre(l=new int[n]; // luu vet
ArrayList<Integer> s = new ArraylList<Integer>();
for (int i = 0; i <n; i++) {
if(il=start){
check[i] = @; //dinh i chua xet
...
int d[]=new int[n];
int pre[]=new int[n];
//kiem tra dinh da xet
// khoang cach ngan nhat hien tai
// luu vet
...
if (u!=i && check [i] == 0) {
if (d[i]>d[u]+a [u] [i]) {
d[i]=d[u]+a [u] [i]; pre[i]=u;
}
}
}
count++;
} while (count<=n);
//export the shortest path from vertex start //print the shortest path
